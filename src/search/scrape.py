...
class Scraper:
    ...
    async def scrape_url(url: str, skip_errors: bool = True) -> str:
        ...
        if 'text/html' in content_type:
            ...
            if content:
                logger.info(f"Successfully extracted HTML content from {url}, size: {len(content)} chars")
            else:
        ...
        if 'application/pdf' in content_type:
            try:
                ...
                logger.info(f"Successfully extracted PDF content from {url}, size: {len(text)} chars")
                return text
            except Exception as e:
